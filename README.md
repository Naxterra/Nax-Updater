...
- the interactive installer is now a WiX Burn setup EXE with a verified fixed-size native window; the embedded MSI runs without its unreliable internal UI, while standalone MSI artifacts use `WixUI_InstallDir` and `ProgramFiles64Folder` for enterprise deployment.
...
./scripts/package-release.ps1 -Version 0.15.6
```
